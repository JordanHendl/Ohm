...
auto Vulkan::System::setDebugParameter(std::string_view str) Ohm_NOEXCEPT -> void {
...
auto Vulkan::Memory::heaps(int gpu) Ohm_NOEXCEPT -> const std::vector<GpuMemoryHeap>& {
...
auto Vulkan::Memory::type(int32_t handle) Ohm_NOEXCEPT -> HeapType {
  ...
  OhmException(mem.initialized(), Error::APIError, "Attempting to use memory object that is not initialized.");
  return mem.type;
}
...
auto Vulkan::Memory::offset(int32_t handle, size_t offset) Ohm_NOEXCEPT -> size_t {
  OhmException(handle < 0, Error::APIError, "Invalid handle passed to API.");
  auto& parent = ovk::system().memory[handle];
  OhmException(parent.initialized(), Error::APIError, "Attempting to use memory object that is not initialized.");
  auto index = 0;
  for (auto& mem : ovk::system().memory) {
    if (!mem.initialized()) {
...
auto Vulkan::Array::create(int gpu, size_t num_elmts, size_t elm_size) Ohm_NOEXCEPT -> int32_t {
    auto& device = ovk::system().devices[gpu];
    auto index = 0;
...
auto Vulkan::Array::destroy(int32_t handle) Ohm_NOEXCEPT -> void {
  OhmException(handle < 0, Error::APIError, "Attempting to delete an invalid array handle.");
  auto& buf = ovk::system().buffer[handle];
  auto tmp = ovk::Buffer();
  
  OhmException(buf.initialized(), Error::APIError, "Attempting to use array object that is not initialized.");
  tmp = std::move(buf);
}

auto Vulkan::Array::required(int32_t handle) Ohm_NOEXCEPT -> size_t {
  OhmException(handle < 0, Error::APIError, "Attempting to query an invalid array handle.");
  auto& buf = ovk::system().buffer[handle];
...
auto Vulkan::Array::bind(int32_t array_handle, int32_t memory_handle) Ohm_NOEXCEPT -> void {
  OhmException(array_handle < 0, Error::APIError, "Attempting to bind to an invalid array handle.");
  OhmException(memory_handle < 0, Error::APIError, "Attempting to bind an invalid memory handle.");
...
auto Vulkan::Commands::create(int gpu, QueueType type) Ohm_NOEXCEPT -> int32_t {
  ...
  OhmException(true, Error::APIError, "Too many commands. API has run out of allocation.");
  return -1;
}

auto Vulkan::Commands::destroy(int32_t handle) Ohm_NOEXCEPT -> void {
  OhmException(handle < 0, Error::APIError, "Attempting to destroy an invalid commands handle.");
  auto& cmd = ovk::system().commands[handle];
  OhmException(cmd.initialized(), Error::APIError, "Attempting to use object that is not initialized.");
...
auto Vulkan::Commands::copyArray(int32_t handle, int32_t src, int32_t dst, size_t count) Ohm_NOEXCEPT -> void {
  OhmException(handle < 0, Error::APIError, "Attempting to use an invalid commands handle.");
  OhmException(src < 0, Error::APIError, "Attempting to use an invalid src array handle.");
  OhmException(dst < 0, Error::APIError, "Attempting to use an invalid dst array handle.");
  auto& cmd = ovk::system().commands[handle];
  OhmException(cmd.initialized(), Error::APIError, "Attempting to use object that is not initialized.");
...
auto Vulkan::Commands::copyArray(int32_t handle, int32_t src, void* dst, size_t count) Ohm_NOEXCEPT -> void {
  OhmException(handle < 0, Error::APIError, "Attempting to use an invalid commands handle.");
  OhmException(src < 0, Error::APIError, "Attempting to use an invalid src array handle.");
  OhmException(dst == nullptr, Error::APIError, "Attempting to use an invalid dst ptr.");
...
auto Vulkan::Commands::copyArray(int32_t handle, const void* src, int32_t dst, size_t count) Ohm_NOEXCEPT -> void {
  OhmException(handle < 0, Error::APIError, "Attempting to use an invalid commands handle.");
  OhmException(src == nullptr, Error::APIError, "Attempting to use an invalid src pointer.");
  OhmException(dst < 0, Error::APIError, "Attempting to use an invalid dst array handle.");
