  ...
  namespace ovk
  {
    ...
    class CommandBuffer
    {
      public:
        ...
        auto blit( RenderPass& src, Swapchain& dst, Filter filter, unsigned subpass = 0, unsigned framebuffer = 0 ) -> void ;
        auto blit( RenderPass& src, RenderPass& dst, Filter filter, unsigned subpass = 0, unsigned framebuffer = 0 ) -> void ;
        ...
        auto draw( const Buffer& indices, const Buffer& vertices, unsigned instance_count = 1 ) -> void ;
...
//        auto transitionSingle( Image& texture, vk::CommandBuffer cmd, vk::ImageLayout layout ) -> void ;
        ...
        inline auto current() const {return this->m_cmd_buffers[ this->m_current_id ];}
      ...
      private:
        using RecordFunction = std::function<void(vk::CommandBuffer& buffer, unsigned index)> ;
        struct CmdBuffSync
        {
          bool          signaled            = false ;
