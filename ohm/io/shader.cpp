...
namespace io {
inline namespace v1 {
auto type_from_name(const std::string& type) -> Shader::Type {
...
auto convert(Shader::Type type) -> shaderc_shader_kind {
  switch (type) {
...
struct Shader::ShaderData {
  ...
  auto preprocess(std::string_view name, shaderc_shader_kind kind,
                  std::string_view src) -> std::string;
  auto assemblize(std::string_view name, shaderc_shader_kind kind,
                  std::string_view src, bool optimize = false) -> std::string;
  auto assemble(shaderc_shader_kind kind, std::string_view src,
