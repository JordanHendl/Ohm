...
namespace ohm {
  ...
  Array<API, Type, Allocator>::Array(Memory<API, Allocator>&& mv, size_t count) {
    ...
    OhmException(!enough_memory, Error::LogicError, "Attempting to create a buffer from memory that has not allocated enough.");
    if(enough_memory) {
  ...
  template<typename API, typename Type, class Allocator>
  auto Array<API, Type, Allocator>::operator=(Array<API, Type, Allocator>&& mv) -> Array<API, Type, Allocator>& {
  ...
  template<typename API, typename Type, class Allocator>
  auto Array<API, Type, Allocator>::memory() const -> const Memory<API, Allocator>& {
 ...
 * Array::required(array_handle) -> size_t (of required memory to bind to buffer)
 * Array::bind(array_handle, memory_handle) -> void
 */