...
namespace ohm
{
  namespace ovk
  {
    ...
    Buffer::Buffer(Device& device, size_t count, size_t size)
    {
      ...
      //@JH TODO Make this configurable. Will work for now, but is not the most efficient.
      ...
      this->m_element_size = size / count ;
      this->createBuffer( size ) ;
      this->m_requirements = device.device().getBufferMemoryRequirements( this->m_buffer, device.dispatch() ) ;
    }
    
    Buffer::Buffer(Buffer&& mv) {
      *this = std::move(mv);
    }
    ...
    Buffer::~Buffer()
    {
      if( this->initialized() )
      {
        ...
        device.destroy( this->m_buffer, this->m_device->allocationCB(), this->m_device->dispatch() ) ;
      }
    }
    ...
    void Buffer::createBuffer( unsigned size )
    {
      ...
      this->m_buffer = error(this->m_device->device().createBuffer( info, this->m_device->allocationCB(), this->m_device->dispatch())) ;
    }

    void Buffer::bind( Memory& memory )
    {
      OhmException(memory.size() < this->m_requirements.size, Error::APIError, "Attempting to bind memory that is of inadequate size.");
      error(this->m_device->device().bindBufferMemory( this->m_buffer, memory.memory, memory.offset, this->m_device->dispatch() )) ;
